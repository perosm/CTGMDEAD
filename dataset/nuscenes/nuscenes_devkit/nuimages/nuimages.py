...
def annotation_name(
    attributes: List[dict], category_name: str, with_attributes: bool = False
) -> str:
    """
...
def get_font(fonts_valid: List[str] = None, font_size: int = 15) -> ImageFont:
    ...
    fonts_in_sys = matplotlib.font_manager.findSystemFonts(
    ...
    warnings.warn(
        "No suitable fonts were found in your system. "
...
def name_to_index_mapping(category: List[dict]) -> Dict[str, int]:
    ...
    assert max(name_to_index.values()) < 24, (
        ...
        "but there should be 23. Please check your category.json".format(
            max(name_to_index.values())
        )
    )
...
class NuImages:
    ...
    def __init__(
        ...
        if verbose:
            print(
                "Done loading in {:.3f} seconds (lazy={}).\n======".format(
                    time.time() - start_time, self.lazy
                )
            )
    ...
    def __getattr__(self, attr_name: str) -> Any:
        ...
        if attr_name in self.table_names:
            return self._load_lazy(
                attr_name, lambda tab_name: self.__load_table__(tab_name)
            )
        else:
    ...
    def __load_table__(self, table_name) -> List[dict]:
        ...
        if self.verbose:
            print(
                "Loaded {} {}(s) in {:.3f}s,".format(
                    len(table), table_name, end_time - start_time
                )
            )
    ...
    def shortcut(
        ...
        if src_table == "sample_data" and tgt_table == "sensor":
            ...
            src_table == "object_ann" or src_table == "surface_ann"
        ) and tgt_table == "sample":
            ...
            raise Exception(
                "Error: Shortcut from %s to %s not implemented!"
                % (src_table, tgt_table)
            )
    ...
    def check_sweeps(self, filename: str) -> None:
        ...
        assert filename.startswith("samples") or filename.startswith("sweeps"), (
            "Error: You passed an incorrect filename to check_sweeps(). Please use sample_data["
            "filename"
            "]."
        )
        ...
        if "sweeps" in filename:
            ...
            if not osp.isdir(sweeps_dir):
                raise Exception(
                    'Error: You are missing the "%s" directory! The devkit generally works without this '
    ...
    def list_attributes(self, sort_by: str = "freq") -> None:
        ...
        if sort_by == "name":
            sort_order = [
                i
                for (i, _) in sorted(
                    enumerate(self.attribute), key=lambda x: x[1]["name"]
                )
            ]
        elif sort_by == "freq":
            attribute_freqs_order = [
                attribute_freqs[c["token"]] for c in self.attribute
            ]
            sort_order = [
                i
                for (i, _) in sorted(
                    enumerate(attribute_freqs_order), key=lambda x: x[1], reverse=True
                )
            ]
        else:
        ...
        for s in sort_order:
            ...
            print(
                format_str.format(
                    attribute_freqs[attribute["token"]],
                    attribute["name"],
    ...
    def list_cameras(self) -> None:
        ...
        for channel in cs_freqs.keys():
            ...
            print(format_str.format(cs_freq, channel_freq, channel))

    def list_categories(
        self, sample_tokens: List[str] = None, sort_by: str = "object_freq"
    ) -> None:
        """
        ...
        if sort_by == "name":
            sort_order = [
                i
                for (i, _) in sorted(
                    enumerate(self.category), key=lambda x: x[1]["name"]
                )
            ]
            ...
            sort_order = [
                i
                for (i, _) in sorted(
                    enumerate(object_freqs_order), key=lambda x: x[1], reverse=True
                )
            ]
            ...
            sort_order = [
                i
                for (i, _) in sorted(
                    enumerate(surface_freqs_order), key=lambda x: x[1], reverse=True
                )
            ]
        else:
            raise Exception("Error: Invalid sorting criterion %s!" % sort_by)
        ...
        for s in sort_order:
            ...
            print(format_str.format(object_freq, surface_freq, name, description))

    def list_anns(
        self, sample_token: str, verbose: bool = True
        ...
        surface_anns = [
            o for o in self.surface_ann if o["sample_data_token"] == key_camera_token
        ]
        ...
        if verbose:
            ...
            for object_ann in object_anns:
                ...
                attribute_names = [
                    self.get("attribute", at)["name"]
                    for at in object_ann["attribute_tokens"]
                ]
                print(
                    "{} {} {}".format(
                        object_ann["token"], category["name"], attribute_names
                    )
                )
    ...
    def list_logs(self) -> None:
        ...
        for log in self.log:
            ...
            print(format_str.format(sample_freq, logfile, location))
    ...
    def list_sample_content(self, sample_token: str) -> None:
        ...
        timestamps = np.array(
            [
                self.get("sample_data", sd_token)["timestamp"]
                for sd_token in sample_data_tokens
            ]
        )
    ...
    def list_sample_data_histogram(self) -> None:
        ...
        if bin_range == 0:
            ...
            freqs = bins[
                1:
            ]  # To get the frequency we need to use the right side of the bin.
    ...
    def get_sample_content(self, sample_token: str) -> List[str]:
        """
    ...
    def get_ego_pose_data(
        self, sample_token: str, attribute_name: str = "translation"
    ...
    def get_trajectory(
        self, sample_token: str, rotation_yaw: float = 0.0, center_key_pose: bool = True
        ...
        if rotation_yaw is not None:
            rotation = key_rotation.inverse * Quaternion(
                axis=[0, 0, 1], angle=np.pi / 2 - rotation_yaw
            )
    ...
    def get_segmentation(self, sd_token: str) -> Tuple[np.ndarray, np.ndarray]:
        ...
        assert sample_data[
            "is_key_frame"
        ], "Error: Cannot render annotations for non keyframes!"
    ...
    def render_image(
        ...
        if not sample_data["is_key_frame"]:
            ...
            assert (
                not with_attributes
            ), "Error: Cannot render attributes for non keyframes!"
        if with_attributes:
            assert (
                with_category
            ), "In order to set with_attributes=True, with_category must be True."
        ...
        if annotation_type != "none":
            if annotation_type == "all" or annotation_type == "surfaces":
                ...
                if surface_tokens is not None:
                    ...
                    assert set(surface_tokens).issubset(
                        sd_surface_tokens
                ...
                for ann in surface_anns:
                    ...
                    draw.bitmap(
                        (0, 0), Image.fromarray(mask * 128), fill=tuple(color + (128,))
                    )
            ...
            if annotation_type == "all" or annotation_type == "objects":
                ...
                if object_tokens is not None:
                    ...
                    assert set(object_tokens).issubset(
                        sd_object_tokens
                ...
                for ann in object_anns:
                    ...
                    name = annotation_name(
                        attributes, category_name, with_attributes=with_attributes
                    )
        ...
        if out_path is not None:
            plt.savefig(
                out_path, bbox_inches="tight", dpi=2.295 * pix_to_inch, pad_inches=0
            )
            plt.close()
    ...
    def render_trajectory(
        ...
        translations, key_index = self.get_trajectory(
            sample_token, rotation_yaw=rotation_yaw, center_key_pose=center_key_pose
        )
        ...
        plt.plot(
            translations[key_index, 0], translations[key_index, 1], "go", markersize=10
        )  # Key image.
        plt.plot(
            translations[0, 0], translations[0, 1], "rx", markersize=10
        )  # Start point.
        ...
        plt.xlim(
            [
                translations[key_index, 0] - max_dist,
                translations[key_index, 0] + max_dist,
            ]
        )
        plt.ylim(
            [
                translations[key_index, 1] - max_dist,
                translations[key_index, 1] + max_dist,
            ]
        )
