...
def generate_panoptic_labels(
    nusc: NuScenes, out_dir: str, verbose: bool = False
) -> None:
    """
    ...
    for sample_idx in tqdm(range(num_samples), disable=not verbose):
        ...
        point_path = os.path.join(
            nusc.dataroot, nusc.get("sample_data", lidar_token)["filename"]
        )
        ...
        for ann_token in curr_sample["anns"]:
            ...
            if not inst_tok2id[scene_token]:
                ...
                inst_tok2id[scene_token][instance_token] = (
                    len(inst_tok2id[scene_token]) + 1
                )
            instance_id = inst_tok2id[scene_token][instance_token]
            _, boxes, _ = nusc.get_sample_data(
                lidar_token, selected_anntokens=[ann_token]
            )
            indices = np.where(points_in_box(boxes[0], lidar_seg.points[:, :3].T))[0]

            for index in range(len(indices)):
                if (
                    lidar_seg.labels[indices[index]]
                    == cat_name_to_idx[ann["category_name"]]
                ):
                    panop_labels[indices[index]] += instance_id
                    overlap_box_count[indices[index]] += 1

        panop_labels[overlap_box_count > 1] = (
            0  # Set pixels overlapped by > 1 boxes to 0.
        )
        ...
        thing_mask = np.logical_and(
            semantic_labels > 0, semantic_labels < STUFF_START_CLASS_ID
        )
        ...
        np.savez_compressed(
            os.path.join(panoptic_dir, panoptic_file),
            data=panop_labels.astype(np.uint16),
        )
...
def main():
    ...
    parser.add_argument(
        "--verbose", type=bool, default=True, help="Whether to print to stdout."
    )
    parser.add_argument(
        "--out_dir",
        type=str,
        default=None,
        help="Folder to write the panoptic labels to.",
    )
    args = parser.parse_args()
    out_dir = (
        args.out_dir
        if args.out_dir is not None
        else f"Panoptic-nuScenes-{args.version}"
    )
    ...
    print(
        f"Panoptic ground truths saved at {args.out_dir}. \nFinished panoptic ground truth generation."
    )
