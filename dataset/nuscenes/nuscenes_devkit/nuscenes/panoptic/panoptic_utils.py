...
from dataset.nuscenes.nuscenes_devkit.nuscenes.utils.data_io import load_bin_file
...
def generate_panoptic_colors(
    colormap: Dict[str, Iterable[int]],
    name2idx: Dict[str, int],
    max_instances: int = 32000,
) -> np.ndarray:
    """
...
def paint_panop_points_label(
    panoptic_labels_filename: str,
    ...
    panoptic_labels = load_bin_file(
        panoptic_labels_filename, type="panoptic"
    )  # [num_points]
    ...
    if filter_panoptic_labels is not None:
        ...
        assert isinstance(
            filter_panoptic_labels, (list, np.ndarray)
...
def get_frame_panoptic_instances(
    panoptic_label: np.ndarray, frame_id: int = None
) -> np.ndarray:
    """
    ...
    cat_inst_count = np.concatenate(
        [inst_count[:, 0:1] // 1000, inst_count[:, 0:1] % 1000, inst_count[:, 1:]],
        axis=1,
    ).astype(np.int32)

    if frame_id is not None:
        frame_id_col = np.full(
            (cat_inst_count.shape[0], 1), fill_value=frame_id, dtype=np.int32
        )
...
def get_panoptic_instances_stats(
    scene_inst_stats: Dict[str, np.ndarray],
    ...
    for scene_id, scene_data in enumerate(scene_inst_stats.values()):
        scene_id_col = np.full(
            (scene_data.shape[0], 1), fill_value=scene_id, dtype=np.int32
        )
        scene_data = np.concatenate([scene_id_col, scene_data], axis=1)
        data = np.concatenate([data, scene_data], axis=0)

    thing_row_mask = np.logical_and(
        data[:, 2] > 0, data[:, 2] < STUFF_START_CLASS_ID
    )  # thing instances only.
    ...
    inst_num_each_frame = np.array(
        np.unique(1000 * data_thing[:, 0] + data_thing[:, 1], return_counts=True)
    )[1]
    mean_inst_num_per_frame, std_inst_num_per_frame = np.mean(
        inst_num_each_frame
    ), np.std(inst_num_each_frame)
    total_num_sample_annotations = np.sum(inst_num_each_frame)

    per_frame_panoptic_stats = {
        "per_frame_num_instances": (mean_inst_num_per_frame, std_inst_num_per_frame)
    }
    if get_hist:
        inst_num_per_frame_hist = np.array(
            np.unique(inst_num_each_frame, return_counts=True)
        ).T
        per_frame_panoptic_stats.update(
            {"per_frame_num_instances_hist": inst_num_per_frame_hist}
        )
    ...
    for cat_id in unique_cat_ids:
        ...
        per_cat_inst_frame_count = np.array(
            np.unique(per_cat_unique_inst_ids, return_counts=True)
        )[1]
        num_instances = len(per_cat_inst_frame_count)
        mean_num_frames, std_num_frames = np.mean(per_cat_inst_frame_count), np.std(
            per_cat_inst_frame_count
        )
        per_cat_inst_num_pts = data_thing[per_cat_inst_mask, 4]
        mean_num_pts, std_num_pts = np.mean(per_cat_inst_num_pts), np.std(
            per_cat_inst_num_pts
        )
    ...
    total_num_instances = np.sum(
        [v["num_instances"] for _, v in per_category_panoptic_stats.items()]
    )
    # for completeness of all categories, fill with empty values for categories with zero instances.
    other_cats = [
        name
        for i, name in cat_idx2name.items()
        if 0 < i < STUFF_START_CLASS_ID and i not in unique_cat_ids
    ]
