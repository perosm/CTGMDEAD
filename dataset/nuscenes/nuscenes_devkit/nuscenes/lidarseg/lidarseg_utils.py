...
def get_stats(points_label: np.array, num_classes: int) -> List[int]:
    ...
    lidarseg_counts = [
        0
    ] * num_classes  # Create as many bins as there are classes, and initialize all bins as 0.
    ...
    for class_idx, class_count in zip(ii, indices[ii]):
        lidarseg_counts[
            class_idx
        ] += class_count  # Increment the count for the particular class name.
...
def plt_to_cv2(
    ...
    canvas.draw()
    mat = np.array(canvas.renderer.buffer_rgba()).astype(
        "uint8"
...
def colormap_to_colors(
    colormap: Dict[str, Iterable[int]], name2idx: Dict[str, int]
) -> np.ndarray:
    """
    ...
    colors = (
        np.array(colors) / 255
    )  # Normalize RGB values to be between 0 and 1 for each channel.
    ...
    for i in range(len(colors)):
        if i not in classes_to_display:
            colors[i] = [
                ...
                1.0,
                1.0,
            ]  # Mask labels to be hidden with 1.0 in all channels.
...
def create_lidarseg_legend(
    labels_to_include_in_legend: List[int],
    ...
    for i in range(len(classes)):
        if labels_to_include_in_legend is None or i in labels_to_include_in_legend:
            name = classes[i]
            recs.append(
                mpatches.Rectangle((0, 0), 1, 1, fc=np.array(name2color[name]) / 255)
            )
    ...
    if ax is not None:
        ax.legend(
            recs, classes_final, loc=loc, ncol=ncol, bbox_to_anchor=bbox_to_anchor
        )
    else:
        plt.legend(
            recs, classes_final, loc=loc, ncol=ncol, bbox_to_anchor=bbox_to_anchor
...
def paint_points_label(
    lidarseg_labels_filename: str,
    filter_lidarseg_labels: List[int],
    ...
    if filter_lidarseg_labels is not None:
        ...
        assert isinstance(
            filter_lidarseg_labels, (list, np.ndarray)
