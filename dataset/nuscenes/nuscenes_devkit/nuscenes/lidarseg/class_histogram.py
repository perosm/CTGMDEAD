...
def render_histogram(
    ...
    lidarseg_num_points_per_class = get_lidarseg_num_points_per_class(
        nusc, sort_by=sort_by
    )
    panoptic_num_instances_per_class = get_panoptic_num_instances_per_class(
        nusc, sort_by=sort_by
    )
    ...
    assert (
        lidarseg_num_points_per_class.keys() == panoptic_num_instances_per_class.keys()
    ), "Error: There are {} classes for lidarseg, but {} classes for panoptic.".format(
        len(lidarseg_num_points_per_class.keys()),
        len(panoptic_num_instances_per_class.keys()),
    )
    ...
    cmap = get_colormap()
    colors = [
        "#%02x%02x%02x" % tuple(cmap[cn]) for cn in class_names
    ...
    for ax in axes:
        ax.margins(
            x=0.005
        )  # Add some padding to the left and right limits of the x-axis for aesthetics.
        ax.set_axisbelow(
            True
        )  # Ensure that axis ticks and gridlines will be below all other ploy elements.
    ...
    for i, (histogram, config) in enumerate(histograms_config.items()):
        ...
        assert len(class_names) == len(
            axes[i].get_xticks()
        ), "There are {} classes, but {} are shown on the x-axis".format(
            len(class_names), len(axes[i].get_xticks())
        )
...
def get_lidarseg_num_points_per_class(
    nusc: NuScenes, sort_by: str = "count_desc"
) -> Dict[str, int]:
    """
    ...
    for record_lidarseg in nusc.lidarseg:
        lidarseg_labels_filename = os.path.join(
            nusc.dataroot, record_lidarseg["filename"]
        )
    ...
    if sort_by == "count_desc":
        num_points_per_class = dict(
            sorted(num_points_per_class.items(), key=lambda item: item[1], reverse=True)
        )
    elif sort_by == "count_asc":
        num_points_per_class = dict(
            sorted(num_points_per_class.items(), key=lambda item: item[1])
        )
    elif sort_by == "name":
        ...
        raise Exception(
            ...
            "Only `count_desc`, `count_asc`, `name` or `index` are valid.".format(
                sort_by
            )
        )
...
def get_panoptic_num_instances_per_class(
    nusc: NuScenes, sort_by: str = "count_desc"
) -> Dict[str, int]:
    """
    ...
    if sort_by == "count_desc":
        sequence_wise_instances_per_class = dict(
            sorted(
                sequence_wise_instances_per_class.items(),
        ...
        sequence_wise_instances_per_class = dict(
            sorted(sequence_wise_instances_per_class.items())
        )
    elif sort_by == "index":
        sequence_wise_instances_per_class = dict(
            sequence_wise_instances_per_class.items()
