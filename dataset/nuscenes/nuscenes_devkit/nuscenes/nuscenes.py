...
from dataset.nuscenes.nuscenes_devkit.nuscenes.lidarseg.lidarseg_utils import (
    colormap_to_colors,
    plt_to_cv2,
    get_stats,
...
from dataset.nuscenes.nuscenes_devkit.nuscenes.panoptic.panoptic_utils import (
...
from dataset.nuscenes.nuscenes_devkit.nuscenes.utils.data_io import (
    load_bin_file,
    panoptic_to_lidarseg,
)
...
from dataset.nuscenes.nuscenes_devkit.nuscenes.utils.map_mask import MapMask
...
class NuScenes:
    ...
    def __init__(
        ...
        lidar_tasks = [
            ...
            if osp.exists(osp.join(self.table_root, t + ".json"))
        ]
        ...
        for i, lidar_task in enumerate(lidar_tasks):
            ...
            label_files = os.listdir(
                os.path.join(self.dataroot, lidar_task, self.version)
            )
            num_label_files = len(
                [
                    name
                    for name in label_files
                    if (name.endswith(".bin") or name.endswith(".npz"))
                ]
            )
            num_lidarseg_recs = len(getattr(self, lidar_task))
            assert (
                num_lidarseg_recs == num_label_files
    ...
    def get_sample_lidarseg_stats(
        self,
        sample_token: str,
        sort_by: str = "count",
        lidarseg_preds_bin_path: str = None,
        gt_from: str = "lidarseg",
    ) -> None:
        """
        ...
        assert hasattr(self, gt_from), (
            f"Error: You have no {gt_from} data; unable to get "
            "statistics for segmentation of the point cloud."
        )
        assert sort_by in ["count", "name", "index"], (
            "Error: sort_by can only be one of the following: " "count / name / index."
        )
        ...
        assert ref_sd_record["is_key_frame"], (
            "Error: Only pointclouds which are keyframes have "
        ...
        if lidarseg_preds_bin_path:
            ...
            assert os.path.exists(lidarseg_labels_filename), (
                ...
                "(lidar sample data token {}) from.".format(
                    lidarseg_labels_filename, sample_token, ref_sd_token
                )
            )

            header = "===== Statistics for " + sample_token + " (predictions) ====="
        else:
            assert len(semantic_table) > 0, (
                "Error: There are no ground truth labels found for nuScenes-{} for {}."
            ...
            lidarseg_labels_filename = os.path.join(
                self.dataroot, self.get(gt_from, lidar_sd_token)["filename"]
            )
    ...
    def list_lidarseg_categories(
        self, sort_by: str = "count", gt_from: str = "lidarseg"
    ) -> None:
        self.explorer.list_lidarseg_categories(sort_by=sort_by, gt_from=gt_from)

    def list_panoptic_instances(
        self, sort_by: str = "count", get_hist: bool = False
    ) -> None:
    ...
    def render_scene_channel_lidarseg(
        ...
        self.explorer.render_scene_channel_lidarseg(
            scene_token,
            channel,
    ...
    def render_scene_lidarseg(
        ...
        self.explorer.render_scene_lidarseg(
            scene_token,
            out_path=out_path,
            filter_lidarseg_labels=filter_lidarseg_labels,
            with_anns=with_anns,
...
class NuScenesExplorer:
    ...
    def list_lidarseg_categories(
        ...
        for record_lidarseg in semantic_table:
            lidarseg_labels_filename = osp.join(
                self.nusc.dataroot, record_lidarseg["filename"]
            )
            points_label = load_bin_file(lidarseg_labels_filename, type=gt_from)
            if gt_from == "panoptic":
                points_label = panoptic_to_lidarseg(points_label)
        ...
        lidarseg_counts_dict = dict()
        for i in range(len(lidarseg_counts)):
            lidarseg_counts_dict[self.nusc.lidarseg_idx2name_mapping[i]] = (
                lidarseg_counts[i]
            )
        ...
        for class_name, count in out:
            ...
            total_count += count
        print(
            "Calculated stats for {} point clouds in {:.1f} seconds, total {} points.\n=====".format(
    ...
    def list_panoptic_instances(
        ...
        assert hasattr(
            self.nusc, "panoptic"
        ), f"Error: nuScenes-panoptic not installed!"
        assert sort_by in ["count", "name", "index"], (
            "Error: sort_by can only be one of the following: " "count / name / index."
        )
        ...
        for frame_id, record_panoptic in enumerate(nusc_panoptic):
            panoptic_label_filename = osp.join(
                self.nusc.dataroot, record_panoptic["filename"]
            )
            panoptic_label = load_bin_file(panoptic_label_filename, type="panoptic")
            sample_token = self.nusc.get(
                "sample_data", record_panoptic["sample_data_token"]
            )["sample_token"]
            scene_token = self.nusc.get("sample", sample_token)["scene_token"]
            if scene_token not in scene_inst_stats:
                scene_inst_stats[scene_token] = np.empty((0, 4), dtype=np.int32)
            frame_cat_inst_count = get_frame_panoptic_instances(
                panoptic_label=panoptic_label, frame_id=frame_id
            )
            scene_inst_stats[scene_token] = np.append(
                scene_inst_stats[scene_token], frame_cat_inst_count, axis=0
            )

        panoptic_stats = get_panoptic_instances_stats(
            scene_inst_stats, self.nusc.lidarseg_idx2name_mapping, get_hist
        )
        pm = "\u00b1"
        frame_num_insts = panoptic_stats["per_frame_panoptic_stats"][
            "per_frame_num_instances"
        ]
        print(
            "Per-frame number of instances: {:.0f}{}{:.0f}".format(
                frame_num_insts[0], pm, frame_num_insts[1]
            )
        )
        ...
        if sort_by == "count":
            instance_counts = sorted(
                ...
                key=lambda item: item[1]["num_instances"],
                reverse=True,
            )
        elif sort_by == "name":
        ...
        for cat_name, s in instance_counts:
            print(
                "{}: {} instances, each instance spans to {:.0f}{}{:.0f} frames, with {:.0f}{}{:.0f} points".format(
                    cat_name,
                    s["num_instances"],
                    s["num_frames_per_instance"][0],
                    pm,
                    s["num_frames_per_instance"][1],
                    s["num_points_per_instance"][0],
                    pm,
                    s["num_points_per_instance"][1],
                )
            )

        num_instances, num_sample_annos = (
            panoptic_stats["num_instances"],
            panoptic_stats["num_sample_annotations"],
        )
        print(
    ...
    def map_pointcloud_to_image(
        ...
        if pointsensor["sensor_modality"] == "lidar":
            if show_lidarseg or show_panoptic:
                ...
                assert hasattr(
                    self.nusc, gt_from
                ), f"Error: nuScenes-{gt_from} not installed!"
        ...
        if render_intensity:
            ...
            if lidarseg_labels_filename:
                ...
                coloring = depths
                print(
                    f"Warning: There are no lidarseg labels in {self.nusc.version}. Points will be colored according "
    ...
    def render_pointcloud_in_image(
        ...
        if (
            ...
            and (show_lidarseg or show_panoptic)
            and show_lidarseg_legend
        ):
            ...
            if filter_lidarseg_labels is None:
                if show_lidarseg:
                    ...
                    filter_lidarseg_labels = get_labels_in_coloring(
                        color_legend, coloring
                    )
                else:
                    # Only show legends for all stuff categories for panoptic.
                    filter_lidarseg_labels = stuff_cat_ids(
                        len(self.nusc.lidarseg_name2idx_mapping)
                    )

            if filter_lidarseg_labels and show_panoptic:
                # Only show legends for filtered stuff categories for panoptic.
                stuff_labels = set(
                    stuff_cat_ids(len(self.nusc.lidarseg_name2idx_mapping))
                )
                filter_lidarseg_labels = list(
                    stuff_labels.intersection(set(filter_lidarseg_labels))
                )
    ...
    def render_sample_data(
        ...
        if sensor_modality in ["lidar", "radar"]:
            ...
            if sensor_modality == "lidar" and (show_lidarseg or show_panoptic):
                ...
                if lidarseg_preds_bin_path:
                    sample_token = self.nusc.get("sample_data", sample_data_token)[
                        "sample_token"
                    ]
                    lidarseg_labels_filename = lidarseg_preds_bin_path
                    assert os.path.exists(lidarseg_labels_filename), (
                ...
                if lidarseg_labels_filename:
                    ...
                    if show_lidarseg or show_panoptic:
                        if show_lidarseg:
                            ...
                            colors = paint_panop_points_label(
                                lidarseg_labels_filename,
                                filter_lidarseg_labels,
                                self.nusc.lidarseg_name2idx_mapping,
                                self.nusc.colormap,
                        ...
                        if show_lidarseg_legend:
                            ...
                            if filter_lidarseg_labels and show_panoptic:
                                ...
                                filter_lidarseg_labels = list(
                                    stuff_labels.intersection(
                                        set(filter_lidarseg_labels)
                                    )
                                )
    ...
    def render_egoposes_on_map(
        ...
        log_tokens = [
            log["token"] for log in self.nusc.log if log["location"] == log_location
        ]
        ...
        if with_anns:
            ...
            for box in boxes:
                ...
                if (
                    ...
                    and self.nusc.lidarseg_name2idx_mapping[box.name]
